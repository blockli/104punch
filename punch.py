...
def punch(auth_token):
    ...
    data = {"deviceId": gen_device_id(), "latitude": lat, "longitude": lon}
...
def send_notify(state):
    ...
    data = {"chat_id": f"{chat_id}", "text": now + state}
    requests.post(f"https://api.telegram.org/bot{tel_token}/sendMessage", data=data)
